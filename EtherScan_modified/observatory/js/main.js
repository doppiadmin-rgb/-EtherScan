...
class Observatory {
  constructor() {
    ...
    // Load saved settings
    try {
      const ver = localStorage.getItem('etherscan-settings-version');
      if (ver === SETTINGS_VERSION) {
        const saved = localStorage.getItem('etherscan-observatory-settings');
        if (saved) Object.assign(this.settings, JSON.parse(saved));
      } else {
        localStorage.removeItem('etherscan-observatory-settings');
        localStorage.setItem('etherscan-settings-version', SETTINGS_VERSION);
      }
    } catch {}
